...
public interface AnimalTypeRepository extends JpaRepository<AnimalType,Integer> {
     ...
     @Query(value = "SELECT custom_name FROM animal_custom WHERE custom_type=1 and custom_status=1",nativeQuery = true)
     ...
     @Query(nativeQuery = true,value = "SELECT animal_type_from FROM animal_type WHERE animal_type_name=?1")
     String findProviderByTypeName(String animalTypeName);
}
